...
<body>
                ...
                <form class="space-y-4 md:space-y-6" action="{{ route('register') }}" method="POST">
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="terms" aria-describedby="terms" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-primary-600 dark:ring-offset-gray-800" required="" onchange="document.getElementById('registerBtn').disabled = !this.checked">
                        </div>
                        <div class="ml-3 text-sm">
                            <label for="terms" class="font-light text-gray-500 dark:text-gray-300">I accept the <a class="font-medium text-primary-600 hover:underline dark:text-primary-500" href="#">Terms and Conditions</a></label>
                        </div>
                    </div>
                    <button id="registerBtn" type="submit" disabled class="w-full text-white bg-gray-300 cursor-not-allowed font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-gray-600" class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Create an account</button>
  ...
  checkbox.addEventListener('change', function () {
    if (this.checked) {
      ...
      button.classList.add('bg-primary-600', 'hover:bg-primary-700', 'cursor-pointer');
    } else {
      button.disabled = true;
      button.classList.remove('bg-primary-600', 'hover:bg-primary-700', 'cursor-pointer');
